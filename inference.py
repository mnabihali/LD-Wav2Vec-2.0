...
from utils import LibriSpeechDataset, DataCollatorCTCWithPadding, inference
...
config = {
    ...
    'optimizer': 'Adam',
    "scheduler": {
        'type': 'StepLR', 'step': 25, 'gamma': 0.3
    } if args.train_encada and args.use_steplr else {
        'type': 'LambdaLR',
        'param': {'alpha': 0.20, 'beta': 0.03, 'start': 10, 'end': 1.0, 'scale': 10},
    },
}
...
PATH = "/stek/mohamed/ld_wav2vec2/che_again/model_epoch_11.pth"
model = Wav2Vec2ForCTC.from_pretrained(pretrained_model, **model_config)
model.load_state_dict(torch.load(PATH))   # Don't forget to add the path of you model here
