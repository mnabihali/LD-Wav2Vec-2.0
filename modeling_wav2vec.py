...
class Wav2Vec2ForPreTrainingOutput(ModelOutput):
    ...
    Args: loss (*optional*, returned when `sample_negative_indices` are passed, `torch.FloatTensor` of shape `(1,
    )`): Total loss as the sum of the contrastive loss (L_m) and the diversity loss (L_d) as stated in the [official
    paper](https://arxiv.org/pdf/2006.11477.pdf) . (classification) loss. projected_states (`torch.FloatTensor` of
    shape `(batch_size, sequence_length, config.proj_codevector_dim)`): Hidden-states of the model projected to
    *config.proj_codevector_dim* that can be used to predict the masked projected quantized states.
    projected_quantized_states (`torch.FloatTensor` of shape `(batch_size, sequence_length,
    config.proj_codevector_dim)`): Quantized extracted feature vectors projected to *config.proj_codevector_dim*
    representing the positive target vectors for contrastive loss. hidden_states (`tuple(torch.FloatTensor)`,
    *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
    ...
    `(batch_size, sequence_length, hidden_size)`.
            ...
            `tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when
            `config.output_attentions=True`): Tuple of `torch.FloatTensor` (one for each layer) of shape `(
            batch_size, num_heads, sequence_length, sequence_length)`.

            Attentions weights after the attention softmax, used to compute the weighted average in the
            self-attention heads. contrastive_loss (*optional*, returned when `sample_negative_indices` are passed,
            `torch.FloatTensor` of shape `(1,)`): The contrastive loss (L_m) as stated in the [official paper](
            https://arxiv.org/pdf/2006.11477.pdf) . diversity_loss (*optional*, returned when
            `sample_negative_indices` are passed, `torch.FloatTensor` of shape `(1,)`): The diversity loss (L_d) as
            stated in the [official paper](https://arxiv.org/pdf/2006.11477.pdf) .
    """
...
class PolicyModule(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.layer_norm = nn.LayerNorm(config.conv_dim[-1], eps=config.layer_norm_eps)
        # self.layer_norm1 = nn.LayerNorm(12, eps=config.layer_norm_eps)
        self.conv = nn.Conv1d(in_channels=config.conv_dim[-1], out_channels=64, kernel_size=3, padding=1)
        self.adap_avg_pool = nn.AdaptiveAvgPool1d(12)
        self.lin = nn.Linear(768, 12)
        # self.act = ACT2FN[config.feat_extract_activation]  # It is GELU too
        self.act = nn.GELU()
...
class Wav2Vec2FlashAttention2(Wav2Vec2Attention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # TODO: Should be removed once Flash Attention for RoCm is bumped to 2.1. flash_attn<2.1 generates top-left
        #  aligned causal mask, while what is needed here is bottom-right alignement, that was made default for
        #  flash_attn>=2.1. This attribute is used to handle this difference. Reference:
        #  https://github.com/Dao-AILab/flash-attention/releases/tag/v2.1.0. Beware that with flash_attn<2.1,
...
class Wav2Vec2SdpaAttention(Wav2Vec2Attention):
    ...
    def forward(
        ...
        if output_attentions or layer_head_mask is not None:
            # TODO: Improve this warning with e.g. `model.config._attn_implementation = "manual"` once this is
            #  implemented.
            logger.warning_once(
                "Wav2Vec2Model is using Wav2Vec2SdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` "
                "does not support `output_attentions=True` or `layer_head_mask` not None. Falling back to the manual "
                "attention"
        ...
        # inline conditional assignment in SDPA to support both torch.compile's dynamic shapes and full graph
        # options. An inline conditional prevents dynamic shapes from compiling. The tgt_len > 1 is necessary to
        # match with AttentionMaskConverter.to_causal_4d that does not create a causal mask in case tgt_len == 1.
        is_causal = True if self.is_causal and attention_mask is None and tgt_len > 1 else False

        # NOTE: SDPA with memory-efficient backend is currently (torch==2.1.2) bugged when using non-contiguous
        # inputs and a custom attn_mask, but we are fine here as `_shape` do call `.contiguous()`. Reference:
...
class Wav2Vec2EncoderLayer(nn.Module):
    ...
    def forward(self, hidden_states, gate_value, attention_mask=None, output_attentions=False):
        gate_value = gate_value.unsqueeze(-1).unsqueeze(-1)
        attn_residual = hidden_states
        hidden_states, attn_weights, _ = self.attention(
            hidden_states, attention_mask=attention_mask, output_attentions=output_attentions
        )
        hidden_states = self.dropout(hidden_states)
        hidden_states = attn_residual + gate_value * hidden_states

        hidden_states = self.layer_norm(hidden_states)
        hidden_states = hidden_states + gate_value * self.feed_forward(hidden_states)
...
class Wav2Vec2EncoderStableLayerNorm(nn.Module):
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            skip_the_layer = self.training and i > 0 and (dropout_probability < self.config.layerdrop)
            if not skip_the_layer or deepspeed_zero3_is_enabled:
                # under deepspeed zero3 all gpus must run in sync XXX: could optimize this like synced_gpus in
                # generate_utils but not sure if it's worth the code complication
                if self.gradient_checkpointing and self.training:
                    layer_outputs = self._gradient_checkpointing_func(
                        layer.__call__,
                        hidden_states,
...
class Wav2Vec2PreTrainedModel(PreTrainedModel):
    ...
    def load_adapter(self, target_lang: str, force_load=True, **kwargs):
        ...
        # make sure that adapter weights are put in exactly the same precision and device placement and overwritten
        # adapter weights
...
class Wav2Vec2Model(Wav2Vec2PreTrainedModel):
    def __init__(self, config: Wav2Vec2Config):
        ...
        self.policy_model = PolicyModule(config)
    ...
    def forward(
        ...
        gates = self.policy_model(extract_features)

        rand_nConf = 2  # used for inference
        #rand_nConf = torch.randint(low=1, high=12, size=(1,)).item()                     # previous high = 12
        _, gateInd_to_use = torch.topk(input=gates, k=rand_nConf, dim=1,
                                       sorted=True)  # Size of gateInd_to_use = [1, rand_nConf], LongTensor

        # print(gateInd_to_use.size(), gateInd_to_use.type()) gateInd_to_use = torch.Tensor( random.sample(range(12),
        # rand_nConf) ).to('cuda', torch.long).reshape(1, -1) # Randomly Using "rand_nConf" transformers

        gates_to_use = torch.zeros_like(gates)
        gates_to_use[torch.arange(gates.size(0)), gateInd_to_use.t()] = 1
        # print(gates_to_use)

        print("Using the top-{} for current batch.".format(rand_nConf))
        # print("Using the Random-{} for current batch.".format(rand_nConf))

        """## For Training with Probabilistic dropping
        # gates_to_use = torch.bernoulli(torch.zeros((extract_features.size(0), 12), device = 'cuda'), p = 0.5)    # size = [b, 12]
        # print("Using probabilistic gates per input with p_drop={}".format(0.5))
        ## ---------------- For inference -- Randomly dropping ("rand_nConf") conformers
        rand_nConf = 2
        if rand_nConf == 12:
            gates_to_use = torch.ones((1, 12), dtype = torch.long).to('cuda')
        else:
            gates_to_use = torch.zeros(1, 12)
            temp = torch.Tensor(random.sample(range(0, 12), rand_nConf)).to('cuda', torch.long).reshape(1, -1)
            gates_to_use[torch.arange(gates_to_use.size(0)), temp.t()] = 1
            print("Using following gates for current input: ", gates_to_use)
        gates_to_use = gates_to_use.cuda()"""
...
class Wav2Vec2ForCTC(Wav2Vec2PreTrainedModel):
    ...
    def tie_weights(self):
        ...
        # Note that `tie_weights` is usually used to tie input and output embedding weights. The method is
        # re-purposed to correctly load adapter layers for Wav2Vec2 so that we do not have to introduce a new API to
        # [`PreTrainedModel`]. While slightly hacky, Wav2Vec2 never has to tie input and output embeddings,
