    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        if self.preprocess:
            ...
            for data in features:
                # Lowercase the transcription (data[2])
                lowercase_text = data[2].lower()
                inputs = self.processor(data[0].numpy().flatten(), sampling_rate=data[1]).input_values[0]
                with self.processor.as_target_processor():
                    labels = self.processor(lowercase_text).input_ids
...
def train_model(model, processor, dataloaders_dict, optimizer, scheduler, metric, num_epochs, log_interval=10, report_wandb=False, val_interval=5):
            ...
            for phase in ['train', 'val']:
                ...
                for step, inputs in enumerate(dataloaders_dict[phase]):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        #print('Predicted:', preds_str)
                        #print('Reference:', labels_str)
    ...
    return model
'''
def train_model(model, processor, dataloaders_dict, optimizer, scheduler, metric, num_epochs, log_interval=5, report_wandb=False, val_interval=5, save_interval=5, save_dir='./checkpoints'):
    #base_epoch = 183 ### I do this to resume from the checkpoint
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    if torch.cuda.device_count() > 1:
        model = nn.DataParallel(model)
    model = model.to(device)
    ...
    with tqdm(total=num_epochs) as pbar:
        for epoch in range(num_epochs):
                ...
                if phase == 'train':
                    if scheduler:
                        if sc_flag:
                            ...
                            scheduler.step()
                    print('Epoch {}/{} | {:^5} |  Loss: {:.4f} WER: {:.4f}'.format(epoch+1, num_epochs, phase, epoch_loss, epoch_wer))
                    if report_wandb:
                        wandb.log({'train/epoch': epoch+1,
                                   'train/epoch_loss': epoch_loss,
                                   'train/epoch_WER': epoch_wer})
            ...
            if (epoch + 1) % save_interval == 0:
                ...
                if torch.cuda.device_count() > 1:
                    print('Now I will save multi GPU')
                    torch.save(model.module.state_dict(), checkpoint_path+'.pth')
                else:
                    torch.save(model.state_dict(), checkpoint_path+'.pth')

                print(f"Model saved at epoch {epoch+1} to {checkpoint_path}")
            
            ### New way to save from the last checkpoint
            #if (epoch + 1) % save_interval == 0:
                #checkpoint_number = epoch + 1 + base_epoch
                #checkpoint_path = os.path.join(save_dir, f"model_epoch_{checkpoint_number}")
            ...
            #print(f"Model saved at epoch {checkpoint_number} to {checkpoint_path}")
